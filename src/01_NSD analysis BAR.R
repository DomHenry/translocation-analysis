...
## Title:    01_NSD analysis BAR.R
## Purpose:  Analyse movement tracks of BAR Egyptian Geese within a net-squared distance
...
## Required for this and subsequent R files
...
barbirds <- tracks_metadata %>%
  filter(site == "BAR" & d_active > 340) %>%
  select(d_active, everything()) %>%
  print(n = 8) %>%
  pull(pttid)
...
dir.create("data output/BAR resident bursts")
dir.create("data output/NSD plots")

# Function to add xy col to sf object -------------------------------------
...
nyears <- c(1,2,3,1,2,3)

## Checks
tracks_metadata %>%
  filter(pttid %in% barbirds) %>%
  select(pttid,d_active) %>%
  mutate(nyears = nyears)
...
tracks <- map2_dfr(.x = barbirds, .y = nyears,
                   .f = import_tracks)
...
walk(bursts,
     .f = function (z){
       ...
       tracks %>%
         ...
         st_write(glue("data output/BAR resident bursts/{z}.shp"),
...
# Create {adehabitat} ltraj object ---------------------------------------
...
## Refine function - when pEst doesn't work then can try again (only targets models that aren't fit)
nsd_ref <- refine(nsd, p.est = pEst(s.d = 400))
sum(!fullmvmt(nsd_ref))
...
## Plot checks
plot.mvmt(nsd[["7713201_birdyear1"]])
plot.mvmts(nsd)

## Write plots to file
pdf("data output/NSD plots/NSD_model_plots_BAR.pdf", width = 5, height = 4)
